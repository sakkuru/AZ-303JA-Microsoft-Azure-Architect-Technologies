...
Adatum Corporation には、物理サーバーと仮想マシンが混在して実行されているオンプレミスのワークロードがいくつかあります。ほとんどのワークロードには、高可用性 SLA の範囲を含む、ある程度の回復性が必要です。ほとんどのワークロードは、クラスターノード間の同期レプリケーションで、Windows Server フェールオーバー クラスタリングまたは Linux Corosyncクラスターと Pacemaker Resource Manager のいずれかを利用します。Adatumは、同等の機能を Azure に実装する方法を決定しようとしています。特に、Adatum エンタープライズ アーキテクチャ チームは、同じデータ センター内および同じリージョンのデータセンター間の高可用性要件に対応する Azure プラットフォームの機能を調査しています。
...
1. Azure portal で、「**az30301a-lb**」 ブレードに戻り、「**インバウンド NAT 規則**] エントリを選択して、TCP ポート 33890 および 33891 を介してリモート デスクトップ経由でそれぞれバックエンド プール VM の最初と 2 番目に接続できるようにする 2 つの規則をメモします。 
...
1. Azure portal で、「**az30301b-lb**」 ブレードに戻り、「**インバウンド NAT 規則**」 エントリを選択して、TCP ポート 33890 および 33891 を介してリモート デスクトップ経由でそれぞれバックエンド プール VM の最初と 2 番目に接続できるようにする 2 つの規則をメモします。 
...
1. **az303001b-lbruletcp80** ブレードの 「**アウトバウンドの送信元ネットワークアドレス変換(SNAT)**」 セクションで、 **(推奨)アウトバウンド規則を使用して、バックエンド プールのメンバーがインターネットにアクセスできるようにします。** を選択し、次に 「**保存**」 を選択します。

1. 「**az30301b-lb**」 ブレードに戻り、「**送信規則**」 エントリを選択し、「**az30301b-lb \| 送信規則**」 ブレード、「**+ 追加**」 を選択します。
...
1. **az30301c-vmss** 概要ブレードで、**場所**と**障害ドメイン**のエントリを確認します。 
...
1. 「**スケール ルール**」 ブレードで、次の設定を指定し、「**追加**」を選択します (他は既定値のままにします)。
    ...
    | VMName 演算子 | **=** |
    | ディメンション値 | **2 選択済み** |
    | メトリックをインスタンス数で割ることを有効にする | **チェックボックスを選択** |
    | 演算子 | **より大きい** |
...
1. **az30301c-vmss** ブレードで、**インスタンス** エントリを選択し、インスタンスの数が増加していることを確認します。
    ...
    > **注**: インスタンスの数が (1 ではなく) 2 増加する場合があります。これは、実行中のインスタンスの最終的な数が 3 である限り予想されることです。 
...
1. 「**az30301c-vmss**」 ブレードで、「**ディスク**」 を選択し、「**+ 新しいディスクを作成し接続する**」 を選択して、次の設定で新しいマネージド ディスクを接続し (他は既定値のままにします)、「**保存**」 を選択します。
